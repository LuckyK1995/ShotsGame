...
function BossHealthBarImpl() {
  ...
  return (
    <div className="absolute top-[140px] left-1/2 -translate-x-1/2 w-80 pointer-events-none z-20">
      ...
      {/* Boss 增益/减益状态显示行（血条正下方，靠左） */}
      {((gameState.bossBuffs?.length || 0) + (gameState.bossDebuffs?.length || 0)) > 0 && (
        <div
          className="flex items-center gap-1 mt-1 flex-wrap"
          style={{ justifyContent: 'flex-start', minHeight: '22px' }}
        >
          {/* 增益 buff 在前（金色边框） */}
          {(gameState.bossBuffs || []).map((b, i) => (
            <EffectIcon
              key={`buff-${b.type}-${i}`}
              type={b.type}
