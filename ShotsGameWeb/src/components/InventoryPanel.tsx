...
import type { ItemStack, ItemRarity, EquipRarity, Equipment } from '../game/types/game';
...
import { LotteryPotOpenModal } from './LotteryPotOpenModal';
import { StatAdjustModal } from './StatAdjustModal';
import { CraftBlueprintModal } from './CraftBlueprintModal';
...
interface InventoryPanelProps {
  ...
  /** 当前视图：'menu'=主界面（normal 类可用），'battle'=战斗中（battle 类可用） */
  view?: 'menu' | 'battle';
}
...
export function InventoryPanel({ engineRef, view = 'battle' }: InventoryPanelProps) {
  ...
  // 批量使用弹窗
  const [batchUseModal, setBatchUseModal] = useState<{ itemId: string; maxCount: number } | null>(null);
  // 抽奖罐开罐弹窗
  const [lotteryPotOpen, setLotteryPotOpen] = useState(false);
  // 装备属性调整/变化器弹窗
  const [statAdjustModal, setStatAdjustModal] = useState<{ mode: 'reroll' | 'transform' } | null>(null);
  // 设计图制作弹窗
  const [craftBlueprintModal, setCraftBlueprintModal] = useState<string | null>(null);
  ...
  // 按页签设置显示格子数：消耗品100 / 材料50 / 任务奖励30（实际仓库共享 STORAGE_CAPACITY.inventory=100）
  const displaySlots = inventoryTab === 'consumable' ? 100
    : inventoryTab === 'material' ? 50
    : 30;
  // 按当前页签过滤物品（消耗=consumable，材料=material，任务奖励=quest）
  // 材料含：武器碎片/装备布片/装备铁片/首饰金属，各 6 品质
  const tabFilteredInventory = useMemo(() => {
    if (inventoryTab === 'consumable') {
      return inventory.filter(s => {
        const type = getItemDef(s.itemId)?.type;
        return type === 'consumable' || type === 'enhancement';
      });
    }
    ...
    // quest：任务奖励（经验书等）
    return inventory.filter(s => getItemDef(s.itemId)?.type === 'quest');
  ...
  const handleHotbarUse = (index: number) => {
    ...
    if (!itemDef || itemDef.type !== 'consumable') return;
    // 抽奖罐：打开专用开罐弹窗，不走 useItem
    if (itemDef.effect === 'lottery_pot') {
      setLotteryPotOpen(true);
      return;
    }
    // 装备属性调整箱/变化器：打开专用弹窗，不走 useItem
    if (itemDef.effect === 'stat_reroll') {
      setStatAdjustModal({ mode: 'reroll' });
      return;
    }
    if (itemDef.effect === 'stat_transform') {
      setStatAdjustModal({ mode: 'transform' });
      return;
    }
    // 设计图：打开制作弹窗，不走 useItem
    if (itemDef.effect === 'craft_blueprint') {
      setCraftBlueprintModal(stack.itemId);
      return;
    }
  ...
  const handleUseItem = () => {
    if (selectedItem && engineRef.current) {
      const itemDef = getItemDef(selectedItem.stack.itemId);
      if (itemDef && (itemDef.type === 'consumable' || itemDef.type === 'quest')) {
        // 抽奖罐：打开专用开罐弹窗，不走 useItem
        if (itemDef.effect === 'lottery_pot') {
          setLotteryPotOpen(true);
          setSelectedItem(null);
        ...
        // 设计图：打开制作弹窗，不走 useItem
        if (itemDef.effect === 'craft_blueprint') {
          setCraftBlueprintModal(selectedItem.stack.itemId);
          setSelectedItem(null);
          return;
        }
  ...
  // 批量使用（仅 normal 类道具）
  const handleBatchUseClick = () => {
    if (selectedItem && engineRef.current) {
      const itemDef = getItemDef(selectedItem.stack.itemId);
      if (itemDef?.category === 'normal') {
        setBatchUseModal({
          itemId: selectedItem.stack.itemId,
          maxCount: selectedItem.stack.count,
        });
      }
    }
  };

  const handleBatchUseConfirm = (count: number) => {
    if (batchUseModal && engineRef.current) {
      const used = engineRef.current.useItemBatch(batchUseModal.itemId, count);
      if (used > 0) {
        setHotbarToast(`成功使用 ${used} 个`);
      }
      setBatchUseModal(null);
      setSortedInventory(null);
      setSelectedItem(null);
    }
  };
  ...
  const handleBatchSell = useCallback(() => {
    ...
    else newFiltered = newInv.filter(s => getItemDef(s.itemId)?.type === 'quest');
  ...
  return (
    ...
    >
          ...
          >
            {displayedItems.map((stack, index) => {
              ...
              return (
                ...
                >
                  {/* 分类标签：战斗/普通/材料（材料无 battle/normal 分类，标记为「材」） */}
                  {(() => {
                      ...
                      return (
                        <span
                          ...
                          style={{
                            ...
                            fontWeight: 700,
                            color: '#FFFFFF',
                            background: 'rgba(176, 96, 224, 0.85)',
                            padding: '0 2px',
                            borderRadius: '2px',
                            lineHeight: '8px',
                            height: '8px',
                    ...
                    const cat = itemDef.category || 'battle';
                    return (
                      <span
                        className="absolute"
                        style={{
        ...
        >
          ...
          >
                      ...
                      return (
                        <span
                          style={{
                            ...
                            fontWeight: 700,
                            color: '#FFFFFF',
                            background: 'rgba(176, 96, 224, 0.6)',
                            border: '1px solid rgba(176, 96, 224, 0.8)',
                            letterSpacing: '0.5px',
                          }}
                        >
                          材料
                    ...
                    return (
                      <span
                        style={{
                          ...
                          border: `1px solid ${cat === 'normal' ? 'rgba(0, 245, 212, 0.8)' : 'rgba(255, 107, 107, 0.8)'}`,
                          letterSpacing: '0.5px',
                        }}
                      >
                        {cat === 'normal' ? '普通' : '战斗'}
            ...
            <div className="flex gap-2 justify-end flex-wrap">
              ...
              {(() => {
                const itemDef = getItemDef(selectedItem.stack.itemId);
                const isUsable = itemDef && (itemDef.type === 'consumable' || itemDef.type === 'quest');
                const category = itemDef?.category || 'battle';
                // battle 类仅战斗中可用；normal 类仅主界面可用
                const isAvailable = isUsable && (
                  (category === 'battle' && view === 'battle') ||
                  (category === 'normal' && view === 'menu')
                );
                const isNormal = isUsable && category === 'normal';
                return (
                  <>
                    <button
                      ...
                      style={{
                        ...
                        fontSize: '9px',
                        fontWeight: 700,
                        color: isAvailable ? neonCyan : '#5A5A7A',
                    ...
                    {/* 批量使用按钮：仅 normal 类道具显示，战斗时置灰 */}
                    {isNormal && selectedItem.source === 'inventory' && (
                      <button
                        className="px-4 py-1.5"
                        style={{
                          background: isAvailable
                          ...
                          fontSize: '9px',
                          fontWeight: 700,
                          color: isAvailable ? neonYellow : '#5A5A7A',
                          boxShadow: isAvailable
                            ? '0 0 8px rgba(255, 217, 61, 0.1)'
                            : 'none',
                          cursor: isAvailable ? 'pointer' : 'not-allowed',
                          opacity: isAvailable ? 1 : 0.5,
                        }}
                        onClick={handleBatchUseClick}
                        disabled={!isAvailable}
                      >
                        批量使用
                      </button>
      ...
      {batchUseModal && (
        <BatchUseModal
          itemId={batchUseModal.itemId}
          maxCount={batchUseModal.maxCount}
          onConfirm={handleBatchUseConfirm}
          onCancel={() => setBatchUseModal(null)}
        />
      )}
      ...
      <LotteryPotOpenModal
        engineRef={engineRef}
        isOpen={lotteryPotOpen}
        onClose={() => { setLotteryPotOpen(false); setSortedInventory(null); }}
      />
      ...
      {statAdjustModal && (
        <StatAdjustModal
          engineRef={engineRef}
          mode={statAdjustModal.mode}
          onClose={() => {
            setStatAdjustModal(null);
            setSortedInventory(null);
          }}
        />
      )}
      ...
      {craftBlueprintModal && (
        <CraftBlueprintModal
          engineRef={engineRef}
          blueprintItemId={craftBlueprintModal}
          onClose={() => {
            setCraftBlueprintModal(null);
            setSortedInventory(null);
          }}
        />
      )}
...
// 批量使用数量选择弹窗（RestartConfirmModal 模板，垂直滑块+输入框+全部使用）
function BatchUseModal({
  itemId,
  maxCount,
  onConfirm,
  onCancel,
    ...
    >
      ...
      >
        ...
        <div className="flex flex-col items-center mb-4">
          ...
          >
            {itemDef?.name} · 拥有 {maxCount} 个
          </p>
        </div>

        {/* 选择区：左侧垂直滑块 + 右侧输入框 */}
        <div className="flex items-center gap-3 mb-4" style={{ height: '100px' }}>
          {/* 垂直滑块 */}
        ...
        <button
          style={{
            ...
            border: `1px solid ${neonYellow}30`,
            borderRadius: '6px',
            fontFamily: '"Rajdhani", "Orbitron", monospace',
            fontSize: '9px',
            fontWeight: 700,
          ...
          onMouseEnter={(e) => {
            e.currentTarget.style.background = 'rgba(255, 217, 61, 0.18)';
          }}
          onMouseLeave={(e) => {
