...
import { STORAGE_CAPACITY } from '../game/data/storageCapacity';
...
const EquipmentPanelImpl: React.FC<EquipmentPanelProps> = ({ onTabChange, engineRef, onShowStats }) => {
  ...
  // 各仓库容量上限（引用共享常量）
  // 已从 storageCapacity.ts 导入：equipment:100, gem:50, enhance:30, enchant:30
  ...
  const handleEquip = useCallback(() => {
    ...
    if (existingIndex >= 0) {
      // 旧装备将回仓：仓库满时拦截
      if (equipmentStorage.length >= STORAGE_CAPACITY.equipment) {
        setLockToast('仓库已满，无法更换装备');
        return;
      }
    ...
    setSelectedItem(null);
  }, [selectedItem, equipment, equipmentStorage, syncBoth, setLockToast]);

  const handleUnequip = useCallback(() => {
    if (!selectedItem || selectedItem.source !== 'equipped') return;
    // 卸下装备回仓：仓库满时拦截
    if (equipmentStorage.length >= STORAGE_CAPACITY.equipment) {
      setLockToast('仓库已满，无法卸下装备');
      return;
    }
        ...
        <div className="grid grid-cols-3 gap-1 place-items-center mx-auto">
          {EQUIP_SLOTS.map((slot) => {
            ...
            return (
              ...
              >
                {marqueeColor && (
                  <svg className="marquee-border" viewBox="0 0 41 41">
                    <rect x="1.25" y="1.25" width="38.5" height="38.5" rx="8" ry="8" />
                  </svg>
                )}
                {equip ? (
                  <>
      ...
      {selectedItem && (
        ...
        >
          ...
          >
            ...
            <div className="relative flex flex-col" style={{ zIndex: 1, flex: 1, minHeight: 0 }}>
            <div className="flex justify-between items-start mb-2" style={{ flexShrink: 0 }}>
            ...
            {/* 主属性 + 词条：三列紧凑布局 */}
            <div
              className="flex flex-col flex-1 overflow-y-auto pt-2"
              style={{ borderTop: '1px solid rgba(176, 38, 255, 0.15)', minHeight: 0 }}
            >
            ...
            {(() => {
              ...
              return (
                <div className="grid grid-cols-3 gap-x-2 mt-1" style={{ borderTop: '1px solid rgba(176, 38, 255, 0.15)', paddingTop: '4px', flexShrink: 0 }}>
                  {/* 强化 */}
            ...
            {/* 操作按钮 */}
            <div className="mt-2 pt-2" style={{ borderTop: '1px solid rgba(176, 38, 255, 0.1)', flexShrink: 0 }}>
