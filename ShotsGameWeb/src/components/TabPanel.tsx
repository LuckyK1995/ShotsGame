import { neonCyan, neonPurple, neonPink, neonBlue, neonRed, neonYellow } from '../theme/colors';
...
export function TabPanel({ tabs, activeTab, onTabChange, onClose, children }: TabPanelProps) {
  ...
  const tabColors: Record<string, { active: string; inactive: string; icon: string }> = {
    character: { active: neonCyan, inactive: '#5A6A7A', icon: neonCyan },
  ...
  // HUD 背景配色：按当前 activeTab 选取主色 + 辅色（与按钮区纹路/颜色不同）
  const hudAccentMap: Record<string, [string, string]> = {
    character: [neonCyan, neonPurple],
    equipment: [neonCyan, neonPink],
    inventory: [neonPink, neonYellow],
    skills: [neonBlue, neonCyan],
    codex: [neonPurple, neonCyan],
    debug: [neonRed, neonCyan],
  };
  const [hudA1, hudA2] = hudAccentMap[activeTab] || [neonPurple, neonCyan];
  ...
  return (
    <div className="h-full flex flex-col relative">
      ...
      >
        {/* HUD 背景：与按钮区不同纹路/颜色（按当前标签着色） */}
        <ModalHudBackground accentColor={hudA1} accentColor2={hudA2} />
