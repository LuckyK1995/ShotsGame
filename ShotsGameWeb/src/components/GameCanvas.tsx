...
export const GameCanvas = forwardRef<GameCanvasHandle>((_, ref) => {
  ...
  const addGameToast = useGameStore(s => s.addGameToast);
  ...
  const actionsRef = useRef({
    ...
    addRareDropNotification, addGameToast, setMails,
  });
  ...
  useEffect(() => {
    ...
    // 注意：不在此处调用 engine.start()，避免主界面也启动战斗循环
    // 战斗由 handleEnterStage → restartWithMode 触发
    // 仅同步初始玩家/状态到 UI
    A.setGameState({ ...engine.gameState }, { ...engine.player });
    A.setPlayer({ ...engine.player });
