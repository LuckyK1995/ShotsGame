...
import { itemSlotStyle, hexToRgba } from '../utils/styles';
...
// 已学习技能的格子样式：使用 epic 级蓝色发光风格（增强边框亮度以对抗战斗背景贴图）
const skillActiveStyle = {
  background: 'radial-gradient(circle at 50% 40%, #2A3A6A 0%, #1A2A50 55%, #101830 100%)',
  border: `2.5px solid ${hexToRgba(neonBlue, 0.9)}`,
  borderRadius: '8px',
  boxShadow: `0 0 14px ${hexToRgba(neonBlue, 0.55)}, inset 0 0 6px ${hexToRgba(neonBlue, 0.25)}`,
} as React.CSSProperties;

// 战斗快捷栏物品格子：基于共享 itemSlotStyle 增强边框与发光，对抗炫酷背景贴图
function itemSlotBrightStyle(rarity?: string): React.CSSProperties {
  const base = itemSlotStyle(rarity);
  const r = (rarity || 'common') as ItemRarity;
  const baseColor = RARITY_COLORS[r] || RARITY_COLORS.common;
  return {
    ...base,
    border: `2.5px solid ${hexToRgba(baseColor, 0.95)}`,
    boxShadow: `0 0 10px ${hexToRgba(baseColor, 0.5)}, inset 0 0 5px ${hexToRgba(baseColor, 0.2)}`,
  };
}

// 战斗快捷栏空格子：增强边框亮度
const emptySlotBrightStyle: React.CSSProperties = {
  background: 'rgba(19, 16, 37, 0.55)',
  border: '2.5px solid rgba(150, 150, 180, 0.5)',
  borderRadius: '8px',
  boxShadow: 'inset 0 0 4px rgba(0, 245, 212, 0.15)',
};
            ...
            return (
              <div
                ...
                style={{
                  width: CELL,
                  height: CELL,
                  ...(stack ? itemSlotBrightStyle(rarity) : emptySlotBrightStyle),
                }}
              >
                {itemDef && (
                  <span
            ...
            return (
              <div
                ...
                style={{
                  width: CELL,
                  height: CELL,
                  ...(skill ? skillActiveStyle : emptySlotBrightStyle),
                }}
              >
                {skill && (
                  <span
