...
function WaveNoticeImpl() {
  ...
  if (showEliteBoss && noticeType) {
    const isPurgatory = noticeType === 'purgatory';
    const isBoss = noticeType === 'boss';
    const title = isPurgatory ? '击败炼狱BOSS吧！' : (isBoss ? 'BOSS来袭！' : '精英来袭！');
    const titleColor = isPurgatory ? '#B026FF' : (isBoss ? '#FF3333' : '#FF6600');
    const subtitleColor = isPurgatory ? '#E066FF' : (isBoss ? '#FF6666' : '#FF9933');
    const subtitle = isPurgatory ? '获取与BOSS同属性的装备' : (isBoss ? '终极挑战' : '精英怪物');
