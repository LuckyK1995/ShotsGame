...
interface GameStore {
  ...
  // 游戏内悬浮提示（道具使用、升级等，自动过期）
  gameToasts: { id: number; icon: string; text: string; color: string }[];
  // 邮件系统
  mails: Mail[];
  ...
  addGameToast: (info: { icon: string; text: string; color: string }) => void;
...
export const useGameStore = create<GameStore>((set) => ({
  ...
  addGameToast: (info) => set((state) => {
    const id = Date.now() + Math.random();
    return { gameToasts: [...state.gameToasts, { ...info, id }].slice(-4) };
  }),
