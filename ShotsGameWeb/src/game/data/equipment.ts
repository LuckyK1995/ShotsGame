import type { Equipment, ItemDef, EquipSlot, EquipRarity, ItemRarity, SetBonus, SetBonusId, EquipmentAffix, ElementType, QualityTier, QualitySetData, Blueprint } from '../types/game';
...
export const SET_BONUSES: Record<SetBonusId, SetBonus> = {
  ...
  survivor: {
    ...
    icon: '🎒',
    effects: [
      { pieces: 2, effect: '金币获取+15%', value: 15, stat: 'goldBonus' },
...
// 品质对应的基础属性数量、词条数量范围、属性攻击概率
const RARITY_STAT_CONFIG: Record<EquipRarity, { baseStatCount: number; affixMin: number; affixMax: number; elementalAttackChance: number }> = {
  common: { baseStatCount: 1, affixMin: 0, affixMax: 0, elementalAttackChance: 0 },
  advanced: { baseStatCount: 2, affixMin: 0, affixMax: 0, elementalAttackChance: 0 },
  fine: { baseStatCount: 3, affixMin: 0, affixMax: 0, elementalAttackChance: 0.3 },
  legendary: { baseStatCount: 4, affixMin: 0, affixMax: 1, elementalAttackChance: 1 },
  epic: { baseStatCount: 4, affixMin: 1, affixMax: 2, elementalAttackChance: 1 },
  mythic: { baseStatCount: 5, affixMin: 2, affixMax: 2, elementalAttackChance: 1 },
};
...
// 生成装备词条（含属性攻击词条）
// 【普通】0词条、【高级】0词条、【精致】0普通词条+30%概率属性攻击
// 【传说】0-1词条+1属性攻击、【史诗】1-2词条+1属性攻击、【神话】2词条+1属性攻击
// 灼烧/中毒/冰冻/雷电伤害词条只出现在精致品质以上
function rollAffixes(rarity: EquipRarity, level: number, _slot: EquipSlot): EquipmentAffix[] {
  ...
  // 灼烧/中毒/冰冻/雷电伤害词条类型集合
  const ELEMENTAL_DAMAGE_TYPES = new Set(['statusBurn', 'statusPoison', 'statusFreeze', 'statusLightning', 'elementalDamage']);
  // 精致品质以上才能出现这些词条
  const allowElementalDamage = rarity === 'fine' || rarity === 'legendary' || rarity === 'epic' || rarity === 'mythic';
  ...
  if (config.affixMax > 0) {
    ...
    if (affixCount > 0) {
      const pool = [...AFFIX_POOL].filter(a => allowElementalDamage || !ELEMENTAL_DAMAGE_TYPES.has(a.type)).sort(() => Math.random() - 0.5);
  ...
  // 2. 属性攻击词条（精致30%概率，传说以上必有，独立于普通词条数量）
  if (Math.random() < config.elementalAttackChance) {
...
export const ITEMS: Record<string, ItemDef> = {
  ...
  // 经验书：使用后获得对应经验值（日常挑战掉落，放入任务奖励仓库）
  exp_book_small: {
    id: 'exp_book_small',
    name: '小型经验书',
    type: 'quest',
  ...
  exp_book_large: {
    ...
    value: 5000,
    icon: '📘',
    description: '使用后获得5000点经验',
    cooldown: 0,
    category: 'normal',
  },
  // 奖励罐子：普通分类，打开获得随机金币奖励
  reward_pot: {
    id: 'reward_pot',
    name: '奖励罐子',
    type: 'consumable',
    ...
    value: 500,
    icon: '🏺',
    description: '打开获得500~2000金币',
    cooldown: 0,
    category: 'normal',
  },
  // 抽奖罐：参考 DNF 土罐玩法，开启带进度条+图案滚动，随机获得一件道具
  lottery_pot: {
    id: 'lottery_pot',
    name: '抽奖罐',
    type: 'consumable',
    ...
    value: 0,
    icon: '🫙',
    description: '开启后随机获得一件道具（金币/经验书/药水/强化卷轴/宝石）',
    cooldown: 0,
    category: 'normal',
  },
  // 装备属性调整箱：在阈值范围内重新随机装备基础属性数值（不改变属性类型）
  stat_reroll_box: {
    id: 'stat_reroll_box',
    name: '装备属性调整箱',
    type: 'consumable',
    ...
    value: 0,
    icon: '🎲',
    description: '使用后在阈值范围内重新随机调整装备基础属性数值（属性类型不变）',
    cooldown: 0,
    category: 'normal',
  },
  // 装备属性变化器：随机变更装备基础属性的类型并重新赋予数值
  stat_transformer: {
    id: 'stat_transformer',
    name: '装备属性变化器',
    type: 'consumable',
    rarity: 'epic',
    effect: 'stat_transform',
    value: 0,
    icon: '🔀',
    description: '使用后随机变更装备基础属性的类型并重新赋予数值',
    cooldown: 0,
    category: 'normal',
  },
};
...
// ===== 材料定义：4 类型 × 6 品质 = 24 种 =====
// 武器碎片：制作对应品质武器（仅需一种材料+金币）
// 装备布片 + 装备铁片：制作对应品质上衣/护肩/下装/腰带/鞋子（需两种材料+金币）
// 首饰金属：制作对应品质戒指/项链/耳环（仅需一种材料+金币）
// 掉落规则：精致及以下=小怪；传说-史诗=精英；史诗-神话=BOSS
const MATERIAL_TYPES = [
  { baseId: 'weapon_fragment', name: '武器碎片', icon: '🔪', desc: '用于制作同品质武器（数量随装备等级提升）' },
  { baseId: 'armor_cloth', name: '装备布片', icon: '🧵', desc: '用于制作同品质防具（布料，需搭配装备铁片）' },
  { baseId: 'armor_iron', name: '装备铁片', icon: '🔩', desc: '用于制作同品质防具（金属，需搭配装备布片）' },
  { baseId: 'jewelry_metal', name: '首饰金属', icon: '🔗', desc: '用于制作同品质首饰（戒指/项链/耳环）' },
] as const;

const MATERIAL_RARITY_LABEL: Record<ItemRarity, string> = {
  common: '普通',
  advanced: '高级',
  fine: '精致',
  legendary: '传说',
...
for (const mat of MATERIAL_TYPES) {
  for (const rarity of ['common', 'advanced', 'fine', 'legendary', 'epic', 'mythic'] as ItemRarity[]) {
    const id = `${mat.baseId}_${rarity}`;
    ITEMS[id] = {
      id,
      name: `${MATERIAL_RARITY_LABEL[rarity]}${mat.name}`,
      type: 'material',
      rarity,
      icon: mat.icon,
...
// 后处理：所有 consumable 道具未显式声明 category 的，默认为 'battle'（仅战斗中可用）
for (const id of Object.keys(ITEMS)) {
  const item = ITEMS[id];
  if (item.type === 'consumable' && !item.category) {
...
// ===== 装备属性阈值与重摇系统 =====
// 基础属性类型 → Equipment 字段名映射（与生成逻辑一致）
const STAT_FIELD_TO_TYPE: Record<string, string> = {
  attack: 'attack',
  health: 'health',
  defense: 'defense',
  critRate: 'critRate',
...
// 计算 (statType, rarity, level) 的属性阈值 [min, max]
// 与 rollBaseStats 的随机范围一致：multiplier ∈ [0.85, 1.15]
export function getStatThreshold(rarity: EquipRarity, level: number, statType: string): { min: number; max: number } {
  const baseValue = BASE_STAT_VALUES[statType];
  if (baseValue === undefined) return { min: 0, max: 0 };
  const rarityMult = RARITY_MULT[rarity];
  const levelMult = 1 + (level - 1) * 0.13;
  const min = Math.max(1, Math.floor(baseValue * rarityMult * levelMult * 0.85));
  let max = Math.max(min, Math.floor(baseValue * rarityMult * levelMult * 1.15));
  if (statType === 'range') {
    max = Math.min(max, RANGE_CAP[rarity]);
  }
  return { min, max };
}

// 获取装备当前拥有的基础属性类型列表
export function getEquipmentBaseStats(equip: Equipment): { type: string; field: string; value: number }[] {
  const result: { type: string; field: string; value: number }[] = [];
  for (const [field, type] of Object.entries(STAT_FIELD_TO_TYPE)) {
    const v = (equip as any)[field];
...
// 装备属性调整箱：保持属性类型不变，在阈值范围内重新随机数值
export function rerollEquipmentStats(equip: Equipment): Equipment {
  const stats = getEquipmentBaseStats(equip);
  const newEquip = { ...equip };
  // 清空原有基础属性字段
  for (const field of Object.keys(STAT_FIELD_TO_TYPE)) {
    (newEquip as any)[field] = 0;
  }
  for (const s of stats) {
    const { min, max } = getStatThreshold(equip.rarity, equip.level, s.type);
    const value = min + Math.floor(Math.random() * (max - min + 1));
    (newEquip as any)[s.field] = value;
  }
  return newEquip;
}

// 装备属性变化器：随机变更属性类型并重新赋予数值（数量不变，类型随机）
export function transformEquipmentStats(equip: Equipment): Equipment {
  const stats = getEquipmentBaseStats(equip);
  ...
  // 重新随机选取 count 个不同属性类型
  const types = [...BASE_STAT_TYPES].sort(() => Math.random() - 0.5).slice(0, count);
  for (const type of types) {
    const field = STAT_FIELD_TO_TYPE[type];
    const { min, max } = getStatThreshold(equip.rarity, equip.level, type);
    const value = min + Math.floor(Math.random() * (max - min + 1));
    (newEquip as any)[field] = value;
  }
  return newEquip;
}

// ===== 装备商人：可购买等级档位 =====
// 规则：起始档位 = (floor(玩家等级/10)+1)*10，向上 5 档（每 10 级一档，上限 = 玩家等级+50）
// 例：Lv.53 → 60/70/80/90/100；Lv.50 → 60/70/80/90/100；Lv.1-9 → 10/20/30/40/50
export function getMerchantTiers(playerLevel: number): number[] {
  const startLevel = Math.max(10, (Math.floor(playerLevel / 10) + 1) * 10);
  const maxLevel = playerLevel + 50;
  const tiers: number[] = [];
  for (let lvl = startLevel; lvl <= maxLevel && tiers.length < 5; lvl += 10) {
    if (lvl >= 10) tiers.push(lvl);
  }
  return tiers;
}

// 装备商人定价（普通-精致装备）
const MERCHANT_PRICE: Record<EquipRarity, number> = {
  common: 100, advanced: 300, fine: 800, legendary: 2000, epic: 5000, mythic: 12000,
};

// 第一页：生成普通-精致装备全套（3 品质 × 9 部位 = 27 件，购买不限次）
export function generateMerchantEquipmentSet(tierLevel: number): { id: string; equipment: Equipment; price: number; sold: boolean }[] {
  const rarities: EquipRarity[] = ['common', 'advanced', 'fine'];
  const result: { id: string; equipment: Equipment; price: number; sold: boolean }[] = [];
  for (const slot of EQUIP_SLOTS) {
    for (const rarity of rarities) {
      const equip = createEquipment(slot, rarity, tierLevel);
      const base = MERCHANT_PRICE[rarity];
      const price = Math.floor(base * (1 + tierLevel * 0.2));
      result.push({
        id: `meq_${slot}_${rarity}_${tierLevel}`,
        equipment: equip,
        price,
        sold: false,
      });
...
// 制作材料需求计算
function getBlueprintMaterials(slot: EquipSlot, rarity: 'legendary' | 'epic', level: number): { itemId: string; count: number }[] {
  const count = 1 + Math.floor(level / 50); // 等级越高所需材料越多
  if (slot === 'weapon') {
    return [{ itemId: `weapon_fragment_${rarity}`, count }];
  }
  if (slot === 'ring' || slot === 'necklace' || slot === 'earring') {
    return [{ itemId: `jewelry_metal_${rarity}`, count }];
  }
  // 防具需布片+铁片
  return [
    { itemId: `armor_cloth_${rarity}`, count },
    { itemId: `armor_iron_${rarity}`, count },
  ];
}

// 第二页：生成传说-史诗设计图全套（2 品质 × 9 部位 = 18 件，购买不限次）
export function generateBlueprintSet(tierLevel: number): Blueprint[] {
  const rarities: ('legendary' | 'epic')[] = ['legendary', 'epic'];
  const result: Blueprint[] = [];
  for (const slot of EQUIP_SLOTS) {
    for (const rarity of rarities) {
      const mats = getBlueprintMaterials(slot, rarity, tierLevel);
      const base = MERCHANT_PRICE[rarity];
      const blueprintPrice = Math.floor(base * 0.3 * (1 + tierLevel * 0.15));
      const goldCost = Math.floor(base * 0.7 * (1 + tierLevel * 0.2));
      result.push({
        id: `bp_${slot}_${rarity}_${tierLevel}`,
        slot,
        rarity,
        level: tierLevel,
        name: `${RARITY_LABELS[rarity]}${SLOT_LABELS[slot]}设计图`,
        icon: SLOT_ICONS[slot][0],
...
// 设计图道具 ID 模板：bp_<slot>_<rarity>_<level>（与 Blueprint.id 一致，作为 ItemStack.itemId）
// 购买设计图后，会作为消耗品道具加入 inventory，使用时打开制作弹窗
export function getBlueprintItemId(slot: EquipSlot, rarity: 'legendary' | 'epic', level: number): string {
  return `bp_${slot}_${rarity}_${level}`;
}

// 根据设计图道具 ID 反查 Blueprint 元数据（用于制作弹窗显示材料/金币需求）
export function getBlueprintDefFromItemId(itemId: string): Blueprint | null {
  // 反向生成 Blueprint 元数据
  const parts = itemId.split('_');
  if (parts.length !== 4 || parts[0] !== 'bp') return null;
  const slot = parts[1] as EquipSlot;
  const rarity = parts[2] as 'legendary' | 'epic';
  const level = parseInt(parts[3], 10);
  if (!EQUIP_SLOTS.includes(slot) || (rarity !== 'legendary' && rarity !== 'epic') || isNaN(level)) return null;
  const list = generateBlueprintSet(level);
  return list.find(b => b.id === itemId) || null;
}

// 初始化设计图道具定义（运行时插入到 ITEMS，使其作为消耗品出现在物品栏）
// 在文件末尾执行（因为 ITEMS 已定义）
for (const slot of EQUIP_SLOTS) {
  for (const rarity of ['legendary', 'epic'] as const) {
    for (let level = 10; level <= 500; level += 10) {
      const itemId = getBlueprintItemId(slot, rarity, level);
      const bpName = `${RARITY_LABELS[rarity]}${SLOT_LABELS[slot]}设计图`;
      ITEMS[itemId] = {
        id: itemId,
        name: bpName,
        type: 'consumable',
        rarity,
        effect: 'craft_blueprint',
        value: 0,
        icon: SLOT_ICONS[slot][0],
        description: `制作${RARITY_LABELS[rarity]}${SLOT_LABELS[slot]}的设计图，使用后打开制作界面（消耗材料+金币）`,
        cooldown: 0,
        category: 'normal',
      };
    }
